<div>
    ...
    @if ($message1 == true )
        <div class="alert alert-info">{{trans('Services.2')}}</div>
    @endif
    @if ($message2 == true )
        <div class="alert alert-info">{{trans('Services.1')}}</div>
    @endif
    ...
    @else
        <form wire:submit.prevent="saveGroup" autocomplete="off">
            ...
            <div class="form-group">
                <label> {{trans('GroupService.name_group')}}</label>
                <input wire:model="name_group" type="text" name="name_group" class="form-control" required>
            </div>
            <div class="form-group">
                <label> {{trans('GroupService.name_group_en')}}</label>
                <input wire:model="name_group_en" type="text" name="name_group_en" class="form-control" required>
            </div>

            <div class="form-group">
                <label>{{trans('GroupService.notes')}}</label>
                ...
                <div class="card-header">
                    <div class="col-md-12">
                        <button class="btn btn-outline-primary"
                                wire:click.prevent="addService">{{trans('GroupService.add_single_service')}}
                        </button>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tr class="table-primary">
                                <th>{{trans('GroupService.service_name')}}</th>
                                <th width="200">{{trans('GroupService.count')}}</th>
                                <th width="200">{{trans('GroupService.operation')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-sm btn-danger"
                                                wire:click.prevent="removeService({{$index}})">{{trans('Services.5')}}
                                        </button>
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <div class="col-lg-4 ml-auto text-right">
                        <table class="table pull-right">
                            <tr>
                                <td style="color: red">{{trans('Services.6')}}</td>
                                <td>{{ number_format($subtotal, 2) }}</td>
                            </tr>

                            <tr>
                                <td style="color: red">{{trans('Services.7')}}</td>
                                <td width="125">
                            ...
                            <tr>
                                <td style="color: red">{{trans('Services.8')}}</td>
                                <td>
                            ...
                            <tr>
                                <td style="color: red">{{trans('Services.9')}}</td>
                    ...
                    <br/>
                    <div>
                        <input class="btn btn-outline-success" type="submit" value="{{trans('Services.10')}}">
                    </div>
                </div>
            </div>
