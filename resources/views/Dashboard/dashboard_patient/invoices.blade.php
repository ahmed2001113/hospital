...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">{{trans('patients.operation')}}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{trans('patients.invoices')}}</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@include('Dashboard.messages_alert')
    ...
    @else
    @if($invoice->service)
    <td>{{$invoice->service->name_en }}</td>
@else
<td>{{$invoice->group->name_en }}</td>
@endif
    @endif
