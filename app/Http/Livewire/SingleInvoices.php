...
class SingleInvoices extends Component
{
    ...
    public $price,$discount_value = 0 ,$patient_id,$Service_id2,$doctor_id,$section_id,$section_id2,$type,$Service_id,$single_invoice_id,$catchError;
    ...
    public function print($id)
    {
        ...
        return Redirect::route('Print_single_invoices',[
            ...
            'Service_id2' => $single_invoice->Service->name_en,
        ...
        if($this->type == 1){
            ...
            try {
                ...
                // في حالة الاضافة
                    ...
                    // فانكشن الغاء المواعيد التي تم حجزها لما الفاتوره تتعمل
                    $patient = Patient::find($this->patient_id);
                    $appointment_info = AppointmentPatient::where('doctor_id', $this->doctor_id)->where('email', $patient->email)->where('type','مؤكد')->first();
                    if ($appointment_info) {
                        $appointment = AppointmentPatient::find($appointment_info->id);
                        $appointment->update([
                            'type' => 'منتهي'
                        ]);
                    }

                    // كود اضافه رساله في جدول الرسائل
